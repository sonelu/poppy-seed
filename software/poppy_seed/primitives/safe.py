...
class VoltageMonitor(pypot.primitive.LoopPrimitive):
    '''
        This primitive raises an alert by playing a sound when the voltage
        on the bus goes bellow "volt_limit".
        
        On GNU/Linux you can use "aplay" for player
        On MacOS "Darwin" you can use "afplay" for player
        On windows vista+, you can maybe use "start wmplayer"
        '''
    def __init__(self, robot, freq=0.1, volt_limit=7.0, player=None, sound=None):
        pypot.primitive.LoopPrimitive.__init__(self, robot, freq)

        self.volt_limit = volt_limit
        self.sound = sound
        self.watched_motors = self.robot.legs + self.robot.torso + self.robot.arms
    ...
    def check_voltage(self):
        ...
        for m in self.watched_motors:
            if m.present_voltage < min_voltage:
                min_voltage = m.present_voltage
        logger.info('Voltage: {} V'.format(min_voltage))

        if min_voltage < self.volt_limit:
            self.raise_problem(min_voltage)

    def raise_problem(self, min_voltage):
        try:
            subprocess.call([self.player, self.sound])
        except OSError:
            logger.warning('Sound player {} cannot be started'.format(self.player))

        print('Low voltage: {} V'.format(min_voltage))
